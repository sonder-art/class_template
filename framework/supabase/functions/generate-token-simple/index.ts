...
serve(async (req: Request) => {
  ...
  try {
    console.log('🔗 /generate-token-simple endpoint called')
    ...
    // Hash the token using simple crypto instead of bcrypt
    console.log('🔐 Hashing token with crypto.subtle...')
    const tokenHash = await simpleHash(plainToken)
    ...
    console.error('❌ Unexpected error in /generate-token-simple:', error)
    return new Response(
      JSON.stringify({ 
        success: false,
