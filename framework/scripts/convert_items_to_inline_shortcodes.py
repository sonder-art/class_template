#!/usr/bin/env python3
"""
Item Block to Inline Shortcode Converter
This script converts ITEM_START/ITEM_END blocks into Hugo shortcodes with all
parameters embedded inline in the markdown (not stored in separate JSON).
"""
...
class ItemBlockConverter:
    """Converts ITEM_START/ITEM_END blocks to Hugo inline shortcodes"""
    ...
    def convert_item_block_to_inline_shortcode(self, match) -> str:
        """Convert a single ITEM_START/ITEM_END block to an inline shortcode call"""
        ...
        params = []
        params.append(f'constituent_slug="{constituent_slug}"')
        params.append(f'item_id="{item_id}"')
        
        # Add optional parameters
        if metadata.get('points'):
            params.append(f'points="{metadata.get("points")}"')
        if metadata.get('due_date'):
            params.append(f'due_date="{metadata.get("due_date")}"')
        if metadata.get('title'):
            params.append(f'title="{metadata.get("title")}"')
        if metadata.get('delivery_type'):
            params.append(f'delivery_type="{metadata.get("delivery_type")}"')
        if metadata.get('important'):
            params.append(f'important="{metadata.get("important")}"')
        
        # Create shortcode call with all parameters inline
        shortcode = f'{{{{< item-inline {" ".join(params)} >}}}}'
        
        # Ensure proper spacing for Hugo to recognize it as a shortcode
        return f"\n\n{shortcode}\n\n"
    
    def process_file(self, file_path: Path) -> bool:
        """Process a single markdown file to convert ITEM blocks to inline shortcodes"""
        try:
            # Read the file
            ...
            content = self.item_pattern.sub(self.convert_item_block_to_inline_shortcode, content)
            conversions_made += len(self.item_pattern.findall(original_content))
            
            # Convert legacy HOMEWORK_ITEM blocks 
            content = self.legacy_pattern.sub(self.convert_item_block_to_inline_shortcode, content)
            conversions_made += len(self.legacy_pattern.findall(original_content))
            
            # Remove the old item-display shortcode if it exists
            ...
            if content != original_content:
                # Create backup
                backup_path = file_path.with_suffix(file_path.suffix + '.inline-backup')
                ...
                logger.info(f"Converted {conversions_made} item blocks to inline shortcodes in {file_path}")
    ...
    def process_all(self, target_directories: List[str] = None) -> Dict[str, int]:
        ...
        logger.info(f"Inline conversion complete: {results['files_modified']} files modified out of {results['files_processed']} processed")
        return results
...
def main():
    ...
    if results['files_modified'] > 0:
        logger.info("Item block to inline shortcode conversion completed successfully!")
        logger.info(f"Modified files: {converter.processed_files}")
        logger.info("All item values are now directly embedded in markdown files.")
        logger.info("No separate JSON files needed - everything is self-contained!")
    else:
